...
def predict_amrs(
    ...
    if tokens is None:
        ...
        with tqdm(total=total) as bar:
            if add_parents_attention or add_parents_embedding or add_siblings_attention:
                for batch in loader:
                    extra = batch['extra']
                    ii = extra['ids']
                    ids.extend(ii)
                    ...
                    with torch.no_grad():
                        out = model.generate(
                            **x,
                            ...
                            rel_ids = tokenizer.rel_ids,
                            left_id = tokenizer.left_id,
                            right_id = tokenizer.right_id,
                            string_ids = tokenizer.string_ids,
                            length_penalty = 0.6,
                            num_beams=beam_size,
    ...
    for gps, gg in zip(graphs, loader.dataset.graph_origins):
        for gp in gps:
...
def compute_smatch_2(test_path, predictions_path):
    with Path(predictions_path).open() as p, Path(test_path).open() as g:
        score = next(smatch.score_amr_pairs(p, g))
    return score
